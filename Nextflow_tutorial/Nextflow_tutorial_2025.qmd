...
process NORMALIZE_DATA { // Name of the process
    ...
    script: // What the computer is supposed to do
    """
...
* You can use Python, R, bash, or other languages in the script block
...
* The script section contains the code that processes these files
* The output section defines what files the process creates

This same structure works whether you're using Python (like in this example), R for statistics, or bash for running command-line tools. You just change the shebang line and the code in the script block.
...
process ANALYZE_DATA {
    ...
    path "${sample_id}_normalized_data.csv"    // Using the variable to name output file
                                               ...
                                               // The real name of the output file is defined in the script section
...
As you can see, if you want to access the variables in the script section you write `${variable_name}`. This works both for ``val`` and ``path``. 
...
::: {.column-margin}
## What Nextflow Actually Executes

When ``PRINT_ACCESSION`` runs:
...
process COUNT_SEQUENCES {
    ...
    script:
    """
    grep -c ">" /path/to/ecoli.fasta
    """
}
```

When ``PROCESS_PAIRED_READS`` runs:
```groovy
process PROCESS_PAIRED_READS {
    input:
    ["sample_1", "sample_1_R1.fastq", "sample_1_R2.fastq"]
    
    output:
    ["sample_1", "sample_1_R1_trimmed.fastq", "sample_1_R2_trimmed.fastq"]
    ...
    trimmomatic PE \
        sample_1_R1.fastq \
        sample_1_R2.fastq \
        sample_1_R1_trimmed.fastq \
        sample_1_R2_trimmed.fastq 
    """
}

```
These examples show what the processes look like when they actually run, with the variables replaced by real values. The values come from the workflow section, which we'll cover next.
:::
...
2. Calculate fold change and apply log2 using the `np.log2(10)` Numpy function
3. Filter data
...
* How to write pandas operations in the script block

## Workflows

::: {.column-margin}
### Process Order Independence
Nextflow doesn't care about the order of items in a channel. If you have 100 samples, Nextflow will process them as resources become available, not necessarily in the order they were added to the channel. This is great for efficiency - if sample 100 can be processed before sample 1, Nextflow won't artificially wait.
:::
...
Nextflow automatically manages dependencies between processes. If Process B needs output from Process A, Nextflow will wait for Process A to finish that specific sample before starting Process B for that sample. Meanwhile, it can start Process B for other samples that are ready. This "smart waiting" maximizes efficiency without sacrificing correctness.
:::
...
::: {.column-margin}
### Process Isolation
Each process runs in its own directory within Nextflow's work directory. This means two processes running the same tool on different samples can't interfere with each other - they have completely separate workspaces. This isolation prevents file naming conflicts and makes debugging easier since each process's files are neatly contained.
:::
...
```
3. `map()` Transforms items in a channel. Often used with tuples to extract specific elements:

```groovy
/ Starting with tuples from fromFilePairs:
Channel.fromFilePairs('/data/*.fasta', size:1)
// Has: ["sample1", "file1.fa"], ["sample2", "file2.fa"]

// Extract just the files using map:
.map { it[1] }
// Now has: "file1.fa", "file2.fa"

// Combine with collect to get all files in a list:
.map { it[1] }.collect()
// Final result: ["file1.fa", "file2.fa"]
````
...
workflow {
    ...
    // Has: file1.fasta, file2.fasta, file3.fasta
    ...
    // Example with fromFilePairs and map:
    genome_pairs = Channel.fromFilePairs('/data/*.fasta', size:1)
    // Has: ["ecoli", "ecoli.fasta"], ["salmonella", "salmonella.fasta"]
    
    // Process one at a time, using just the file:
    ANALYZE_ONE(genome_pairs.map{ it[1] })
    // Processes: "ecoli.fasta", then "salmonella.fasta"
    
    // Collect all files after extracting them from tuples:
    COMPARE_ALL(genome_pairs.map{ it[1] }.collect())
    // Processes: ["ecoli.fasta", "salmonella.fasta"] all at once
}
}
```
...
We'll analyze three hypothetical bacterial genomes located in a data directory:

* ``/storage/genomes/e_coli.fasta`` (E. coli - a common gut bacteria)
* ``/storage/genomes/s_aureus.fasta`` (S. aureus - can cause skin infections)
* ``/storage/genomes/k_pneumoniae.fasta`` (K. pneumoniae - can cause pneumonia)
...
Using this glob pattern ``Channel.fromFilePairs('/storage/genomes/*.fasta', size:1)`` will return a list of tuples:
...
```groovy
[
    ["e_coli", /storage/genomes/e_coli.fasta],
    ["s_aureus",/storage/genomes/s_aureus.fasta],
    ["k_pneumoniae", /storage/genomes/k_pneumoniae.fasta]
]
...
process RUN_RGI {
    ...
    rgi main -i ${genome_file} -o ${species_name}_rgi 
...
process RUN_FASTANI {
    ...
    script:
    """
    # Create a file listing all genomes, one per line
    ls -1 ${genomes} > genome_list.txt

    # Run fastANI using the list file
    fastANI --ql genome_list.txt --rl genome_list.txt -o ani_results.tsv
    """
}

// Third process: Run FastANI
...
::: {.column-margin}
### Using Local Containers & Environments
Nextflow can use containers and environments that already exist on your machine:

For Apptainer:
```groovy
container '/path/to/local/image.sif'    // Local Singularity/Apptainer image
```

For conda:
```groovy
Copyconda '/path/to/local/environment'      // Existing conda environment
```
This is very useful when you have custom containers/environments. It can however make it hard for someone else to reproduce. Depending on how you set it up. 
::: 
...
* Find appropriate containers for RGI and FastANI on quay.io. COMBINE_RGI_OUTPUT only needs Pandas since glob is part of base python.
* Nextflow had a hard time downloading the RGI container. You can refer to this pre-donwloaded container instead `/cephyr/NOBACKUP/groups/bbt045_2025/Resources/Nextflow_tutorial/container/RGI.sif`.
...
* Point Nextflow to the real files on Vera using the params.input_file varible in your workflow. You can read about what is so nice about the `params` [here](#nextflow-parameters).
...
### Nextflow & SLURM Interaction

Nextflow runs as a single "manager" job on SLURM, but dynamically submits individual process jobs:

1. The main Nextflow job:
   - Uses 1 CPU
   - Orchestrates the workflow
   - Submits other jobs to SLURM
   - Monitors job status

2. Process jobs:
   - Each process becomes a separate SLURM job
   - Resources (CPUs, time) defined in process directives
   - Automatically submitted/managed by Nextflow
   - Run in parallel based on dependencies

So while your SLURM script only asks for 1 CPU, Nextflow will submit additional jobs with their own resource requests based on your process definitions.
:::

## Final pipeline

You should now have a functional `main.nf` file. Save it at an appropriate location on Vera. Then copy paste the `nextflow.config` file below. Read it and make sure you understand what the different parts do. 

If you wanted to run the pipeline on your local machine, or on a server wiht no special sheduling mechanism. All you would have to do it to run this simple line of code. 

```bash
nextflow run main.nf
```
We will however run the pipeline on Vera whihc uses SLURM to shedule jobs. We therefor eneed to write an sbatch script. This script will contain thins like the amount of resourses out job needs, how long it should take, out user names, waht is supposed to be done and so on. 
...
// Everything in params can be changed from the comand line using --{varible_name}. 
params {
    // Input fasta files
    input_files = "/cephyr/NOBACKUP/groups/bbt045_2025/Resources/Nextflow_tutorial/genomes/*.fna"

    // Machine options
    cluster_options = "C3SE2024-2-16 -p vera" // Will be used in your account information for Vera. Basically your user name
}
...
process {
    ...
    clusterOptions = params.cluster_options // here we will use the cluster_options varible. 
}

// None of the things below this comment is nessesary

// Procudes a html file wiht some good metrics for resusrse requirements for each process
report { 
    enabled = true
...
// Procudes a html file showing when processes were run, and if they were paralell
timeline {
    enabled = true
...
// Like teh report file, but in a txt format instead
trace {
    enabled = true
...
// Shows processes nad how they are connected
dag {
    enabled = true
...
::: {.column-margin}
### Understanding SLURM Script Lines

- `#!/usr/bin/env bash`: Tells the system to use bash to run this script

- `#SBATCH -A C3SE2024-2-16 -p vera`: 
  - `-A`: Project account ID
  - `-p`: Which partition/cluster to use

- `#SBATCH -J my_first_pipeline`: Sets job name to 'my_first_pipeline'

- `#SBATCH -c 1`: Requests 1 CPU core

- `#SBATCH -t 00:30:00`: Sets 30-minute time limit

- `#SBATCH --error={PATH}/err/job.%J.err`: 
  - Saves error messages to this file
  - `%J` is replaced with job ID

- `#SBATCH --output={PATH}/err/job.%J.out`:
  - Saves standard output to this file
  - `%J` is replaced with job ID

- `module purge`: Removes all loaded modules

- `module load Nextflow`: Loads Nextflow module

- `nextflow run main.nf -resume`: 
  - Runs the pipeline
  - `-resume` continues from last successful step

- The `--cluster_options` flag has a double dash (`--`) prefix instead of just an underscore because it is a `params` variable. 
In our sbatch script, we submit our account ID using the line `#SBATCH -A C3SE2024-2-16 -p vera`. However, Nextflow doesn't directly interact with this - only the SLURM handler does. Since Nextflow works with SLURM by submitting individual jobs (one per item and process), it needs to know our account name when it creates sbatch scripts. You can look in the `nextflow.config` file to see how this variable is handled.
:::
...
nextflow run main.nf \
-resume \
--cluster_options "-A C3SE2024-2-16 -p vera"
```

This the `sbatch`script is a regular bash script, with SLURM instructions (rows with `#SBATCH`), followed by instructions to the regular command line. Call this script wahtever you like, as long as you give iut the file extension `.sh`. 

### Running the pipeline on SLURM

To run the pipeline using SLURM make sure you have properly configured the `main.nf`,  `nextflow.config`, and `sbatch` file. If the `sbatch` script was called `submit_job.sh`:
...
In Nextflow, each process gets its own special workspace - think of it as each process having its own room to work in. By default, these workspaces live in a directory called `work/` in your project folder. Here's why this is helpful:
Each process workspace contains:

* The exact commands that were run (in `.command.sh`)
* Any output the process produced (in `.command.out`)
* Any errors that occurred (in `.command.err`)
* The actual files the process worked with
* The SLURM submission scripts (if you're using SLURM)

Instead of copying files between these workspaces (which would take up a lot of disk space), Nextflow creates symbolic links (symlinks). These symlinks work like shortcuts or pointers to the original files. This is a clever way to let processes access the files they need without duplicating data.

For example, if you look in the work directory, you might see something like:

```bash
work/
  ├── f2/df4b07_runRGI/          # Workspace for runRGI process
  │   ├── .command.sh            # The commands that were run
  │   ├── .command.out           # Standard output
  │   ├── .command.err           # Error messages
  │   ├── input_data -> ../../data/sample.fa   # Symlink to input
  │   └── results.txt            # Output generated by the process
  └── 2e/58df60_fastANI/         # Workspace for fastANI process
      └── ...                    # Similar structure here
```

This organization keeps everything neat and isolated - no process can accidentally interfere with another process's work. It also makes debugging easier because you can see exactly what happened in each step of your workflow.

## Nextflows runtime display

When you run a Nextflow workflow, you'll see something like this appear in your terminal:

```bash
[f2/df4b07] runRGI             | 15 of 15 ✔
[2e/58df60] fastANI            | 1 of 1 ✔
[-        ] COMBINE_RGI_OUTPUT -
```

This is Nextflow's way of showing you the real-time progress of your workflow. When you first start your workflow, these lines will appear one by one, and the numbers will update as tasks complete. Let's break down what you're seeing:
Each line represents a process in your workflow and follows this pattern:


```bash
[unique ID] process name | completed tasks of total tasks status
```

So in the example above:

The first line `[f2/df4b07] runRGI          | 15 of 15 ✔` shows that:

* `[f2/df4b07]` is the unique ID (or hash) Nextflow assigns to track this process. You can use this hash to find the process's work directory
* `runRGI` is the name of the process
* `15 of 15 ✔` tells us all 15 tasks completed successfully (the checkmark ✔ indicates success)


The second line `[2e/58df60] fastANI            | 1 of 1 ✔` shows a simpler process that:

* Had just one task to do
* Completed successfully


The last line `[-        ] COMBINE_RGI_OUTPUT -` shows a process that:

* Hasn't started yet (shown by the - symbols)
* Is waiting for previous processes to complete

## Working working Nextflow and SLURM

When you're running Nextflow on a SLURM cluster, you'll need these basic SLURM commands:
...
# 12345     runRGI   john    R        5:23    1
```

To cancel jobs if needed:
...
When Nextflow runs with SLURM, each process submits its own job. You can monitor their progress in two ways:

1. Through Nextflow's display (the progress we discussed earlier)
2. Through SLURM's logs

Your sbatch script specifies where to save the output and error logs, usually with these lines:
...
Here, `%j` is replaced with the job ID. These files will contain:

* `.err`: Error messages and standard error output
* `.out`: The program's standard output, including the Nextflow progress display

You can look at these files while the job is running using:

```bash
tail -f slurm-12345.err
```
This helps you track progress and debug any issues that might come up during your workflow run.
...
process FILTER_GENES {
    input:
    tuple val(sample_id), path(expression_file)    // Input is a tuple with sample ID and file path

    output:
    path "${sample_id}_filtered_genes.csv"         // Output filename includes the sample ID
    ...
    data['fold_change'] = data['expression_after'] / data['expression_before']
    ...
    filtered_data = data[
        (data['p_value'] <= 0.05) & 
        (np.log2(data['fold_change'].abs()) > 2)
    ]
    
    # Save results with sample ID in filename
    filtered_data.to_csv("${sample_id}_filtered_genes.csv", index=False)
    """
}
```
:::
...
process RUN_RGI {
    input:
    tuple val(species_name), path(genome_file)    // Input tuple with species name and genome file
    
    output:
    path "${species_name}_rgi.txt", emit: rgi_results    // Main RGI results
    path "${species_name}_log.txt", emit: logs           // Log file
    
    script:
    """
    # Run RGI to find resistance genes
    rgi main --input_sequence ${genome_file} --output_file ${species_name}_rgi 
...
process RUN_FASTANI {
    input:
    path(list_genomes)    // Input all genome files at once
    
    output:
    path "ani_results.tsv", emit: ani_results    // FastANI results
...
process COMBINE_RGI_OUTPUT {
    input:
    path RGI_files    // Takes all RGI result files
   
    output:
    path "combined_RGI.csv"    // Combined output file
    ...
    # Identify all files and add them to a list
    list_input_files = glob.glob("*")
    list_df = []

    for df in list_input_files:
        # Extract sample name and read file
        # rsplit splits on the first occurance of "." from the right
        sample_id = df.rsplit('.', 1)[0]
        df_tmp = pd.read_csv(df, sep = "\t")

        # Add column for sample id and append df to list
        df_tmp["Sample_id"] = sample_id
        list_df.append(df_tmp)

    # Combine all dfs in list to a signe df and save
    df_combined = pd.concat(list_df)
    df_combined.to_csv("combined_RGI.csv", index = False)
    """
}

workflow {
    // Create a channel for your input genome files
    genome_ch = Channel.fromFilePairs('/storage/genomes/*.fasta', size: 1)
    
    // Run RGI on each genome
    RUN_RGI(genome_ch)
    
    // Run FastANI to compare all genomes
    // We need to get just the genome files from the tuple and collect them
    RUN_FASTANI(genome_ch.map{ it[1] }.collect())
    
    // Combine all RGI results
    COMBINE_RGI_OUTPUT(RUN_RGI.out.rgi_results.collect())
}
```
:::
...
workflow {
    list_genomes = Channel.fromFilePairs(params.input_files, size: 1)
    runRGI(list_genomes)
    fastANI(list_genomes.map { it[1] }.collect()) 
    COMBINE_RGI_OUTPUT(runRGI.out.ARGs.collect())
}

process runRGI {
    publishDir "${projectDir}/result/rgi", mode: 'copy'
    container '/cephyr/NOBACKUP/groups/bbt045_2025/Resources/Nextflow_tutorial/container/RGI.sif'
    cpus 1
    time '5m'
   
   input:
   tuple val(species_name), path(genome)

   output:
   path "${species_name}_rgi.txt", emit: ARGs
   path "${species_name}_log.txt", emit: log

   script:
   """
   rgi main -i ${genome} -o ${species_name}_rgi
...
process fastANI {
    publishDir "${projectDir}/result/ani", mode: 'copy'
   ...
   # Run fastANI using the list file
   fastANI --ql genome_list.txt --rl genome_list.txt -o ani_results.tsv --threads ${task.cpus}
   """
}
