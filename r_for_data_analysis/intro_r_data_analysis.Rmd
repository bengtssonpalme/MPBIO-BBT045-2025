...
chapter of the *Advanced R* book, at laest to be aware.
...
```{r}
iris %>%
    mutate(log_petal_length = log(Petal.Length))
```
...
```{r}
iris %>% 
    mutate(species_number = sapply(Species,
...
```{r}
PROT_SEQ_YEAST_FILE <- "orf_trans.fasta.gz"
...
GENOME_INFO_SOURCE <- "ftp://ftp.ncbi.nlm.nih.gov/genomes/archive/old_genbank/Bacteria/Halobacterium_sp_uid217/AE004437.gff"

genome_info <- readr::read_tsv(RCurl::getURL(GENOME_INFO_SOURCE),
                              comment = "#",
